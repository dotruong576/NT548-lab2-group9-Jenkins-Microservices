...
function build()
{
  ...
  if [ "$cnb" = true ] || [ "$all" = true ]; then
    ...
    pack $quiet_args --no-color build quyhoangtat-$component:build --builder $builder --path $component_dir --tag $repository/retail-store-$component:$cnb_tag $pack_args

    if [ "$push" = true ] ; then
      msg "Pushing image for ${GREEN}$component${NOFORMAT}..."

      docker push -q $repository/quyhoangtat-$component:$cnb_tag
    fi
  fi
